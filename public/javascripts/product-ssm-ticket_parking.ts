;(() => {
  ...
  const formElement = document.querySelector('#form--product') as HTMLFormElement

  const tagNumberIsValidElement = formElement.querySelector(
    "[name='tagNumber-isValid']"
  ) as HTMLInputElement
  const tagNumberIsValidIconElement = formElement.querySelector(
    '.tagNumber-isValidIcon'
  ) as HTMLSpanElement
  const tagNumberIsInvalidMessageElement = formElement.querySelector(
    '.tagNumber-isInvalidMessage'
  ) as HTMLSpanElement

  const tagNumberPrefixElement = formElement.querySelector(
    "[name='tagNumber-prefix']"
  ) as HTMLSelectElement
  const tagNumberSuffixElement = formElement.querySelector(
    "[name='tagNumber-suffix']"
  ) as HTMLInputElement
  ...
  const checkTagNumber = () => {
    const tagNumber =
      tagNumberPrefixElement.value + ' ' + tagNumberSuffixElement.value
