...
(() => {
  ...
  const concatenateTicketNumber = (ticketNumberElementNames: string[]) => {
    ...
    for (const ticketNumberElementName of ticketNumberElementNames) {
      ticketNumber += " " + (formElement.querySelector("[name='" + ticketNumberElementName + "']") as HTMLInputSelectElement).value;
    }

    return ticketNumber.trim();
  };

  const initializeTicketNumber = (targetElement: HTMLInputElement, ticketNumberElementNames: string[]) => {
    ...
    for (const ticketNumberElementName of ticketNumberElementNames) {
      ...
      const ticketNumberElement = formElement.querySelector("[name='" + ticketNumberElementName + "']") as HTMLInputSelectElement;
  ...
  const fileNumberElement = formElement.querySelector("input[name='fileNumber']") as HTMLInputElement;
