...
window.exports.cart = (() => {
  ...
  let shippingForm: recordTypes.ShippingForm = sessionStorage.getItem(
    SESSION_STORAGE_KEY
  )
  ...
  const addFunction = (productFormElement: HTMLFormElement) => {
  ...
  const cacheContactFunction = () => {
    try {
      shippingForm.fullName = (
        document.querySelector('#shipping_fullName') as HTMLInputElement
      ).value
      shippingForm.address = (
        document.querySelector('#shipping_address') as HTMLInputElement
      ).value
      shippingForm.address2 = (
        document.querySelector('#shipping_address2') as HTMLInputElement
      ).value
      shippingForm.city = (
        document.querySelector('#shipping_city') as HTMLInputElement
      ).value
      shippingForm.province = (
        document.querySelector('#shipping_province') as HTMLInputElement
      ).value
      shippingForm.country = (
        document.querySelector('#shipping_country') as HTMLInputElement
      ).value
      shippingForm.postalCode = (
        document.querySelector('#shipping_postalCode') as HTMLInputElement
      ).value
      shippingForm.phoneNumberDay = (
        document.querySelector('#shipping_phoneNumberDay') as HTMLInputElement
      ).value
      shippingForm.phoneNumberEvening = (
        document.querySelector(
          '#shipping_phoneNumberEvening'
        ) as HTMLInputElement
      ).value
      shippingForm.emailAddress = (
